angular.module('courseWebApp').factory 'Admin', [
    ...
    ($http, $q, $localStorage) ->
        ...
        factory.createTimelog = (date, courseId, canceler = null) ->
            $q (resolve, reject) ->
                $http.post("/api/course/#{courseId}/timelog/create", {
                    date: date
                }, factory.httpConfig(canceler)).then ((response) ->
                    handleSuccessPromise(resolve, reject, response)
                ), (response) ->
        ...
        factory.createScore = (projectScore, canceler = null) ->
            $q (resolve, reject) ->
                $http.post('/api/project/score/create', {
