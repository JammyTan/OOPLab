...
# hack it
    ...
    $scope.computeLocDiff = ()->
      length = $scope.timelogs.length
      if length < 2 ?
        $scope.locDiff = false
      else
        if $scope.timelogs[length-1].loc - $scope.timelogs[length-2].loc == 0
          $scope.locDiff = true
        else
          $scope.locDiff = false
    ...
    $scope.prepareAllGroup = () ->
      $q (resolve, reject) ->
        Student.AllGroup().then ((data) ->
          ...
          $scope.sortGroupsCommitRank()
#          $scope.sortGroupsLocRank()
          $scope.loading = false
          resolve()
        ), (msg) ->
          $scope.loading = false
    ...
    $scope.sortGroupsCommitRank = () ->
      $scope.groupCommitsRank = _.orderBy($scope.groups, [(group)-> if group.project then  return group.project.total_commits else return -1] , ["desc"])

    $scope.sortGroupsLocRank = () ->
      $scope.groupLocRank = _.orderBy($scope.groups, [(group)-> if group.project then  return group.project.total_loc else return -1] , ["desc"])
...
# TODO(Red): group demo score
    ...
    $scope.findGroupCommitsRank = (projectId) ->
      index =  _.findIndex($scope.groupCommitsRank, (group) ->
        if group.project
          return group.project.id == projectId)
      if index == -1
        return "沒有專案"
      return index+1

    $scope.findGroupCommitsAmount = (projectId) ->
      index =  _.findIndex($scope.groupCommitsRank, (group) ->
        if group.project
          return group.project.id == projectId)
      if index == -1
        return ""
      return $scope.groupCommitsRank[index].project.total_commits

    $scope.findGroupLocRank = (projectId) ->
      index =  _.findIndex($scope.groupLocRank, (group) ->
        if group.project
          return group.project.id == projectId)
      if index == -1
        return "沒有專案"
    ...
    $scope.findGroupLocAmount = (projectId) ->
      index =  _.findIndex($scope.groupLocRank, (group) ->
        if group.project
          return group.project.id == projectId)
      if index == -1
        return ""
      return $scope.groupLocRank[index].project.total_loc
])
.filter('studentsName', () ->
  (students) ->
    if not students?
...
.filter('dateRange', () ->
  (timelogs, index) ->
    ...
    tempEndDate = (new Date(timelogs[index].date))
