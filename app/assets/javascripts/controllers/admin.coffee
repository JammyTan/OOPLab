    ...
    ($scope, Admin, Group, $q, $timeout, $state) ->
        ...
        $scope.submitDestroyRollCall = (period, studentId) ->
            ...
            Admin.destroyRollCall(
                ...
                Materialize.toast("已取消登記缺課", 2000)
                $scope.requestLoading = false
                $scope.rollCallReqSuccess.period = period
                $scope.rollCallReqSuccess.studentId = studentId
            ), (msg) ->
        ...
        $scope.submitScore = (number) ->
            $scope.composeProjectScore(number)
            console.log("$scope.project_score", $scope.project_score)
            $('#verify-modal').openModal()
            $scope.verifyCallback = () ->
                Admin.createScore(
                  $scope.project_score
        ...
        $scope.composeProjectScore = (number) ->
          ...
          for group in $scope.groups
            projectScore={}
            projectScore.project_id = group.project.id
            projectScore.point = $scope.transormScoreText(group.project.score_text[number])
            projectScore.no = number
            $scope.project_score.push(projectScore)


        $scope.transormScoreText = (scoreText) ->
          if scoreText == "A+"
            return 90
          else if scoreText == "A"
            return 85
        ...
        $scope.editTimelogAcceptance = (group) ->
            ...
            $scope.verifyCallback = () ->
                ...
                Admin.editTimelogAcceptance(timelog.acceptance, timelog.id).then ((data) ->
                    ...
                    $scope.requestLoading = false
            $scope.verifyFalse = () ->
                timelog.acceptance = !timelog.acceptance
        ...
        $scope.addProjectScoreText = () ->
          for group in $scope.groups
            if group.project
              group.project.total_timelog_cost = Math.ceil(group.project.total_timelog_cost * 10) / 10
              if group.project.average_point
                score1 = $scope.transformAveragePointToText(group.project.average_point[0])
                score2 = $scope.transformAveragePointToText(group.project.average_point[1])
                group.project.score_text = [score1, score2]



        $scope.transformAveragePointToText = (score) ->
          if score >= 90
            return 'A+'
          else if score >= 85
            return 'A'
