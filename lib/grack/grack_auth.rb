    ...
    class Auth < Rack::Auth::Basic
        ...
        def auth!
            ...
            if @auth.provided?
                ...
                @teachingAssistant = authenticate_assistant(login, password)

                if @student and can_handle_project(@project, @student)
                    # Gitlab::ShellEnv.set_env(@user)
                    @env['REMOTE_USER'] = @auth.username
                elsif @teachingAssistant
                    @env['REMOTE_USER'] = @auth.username
                else
                    return unauthorized
                end
        ...
        def authenticate_assistant(login, password)
            assistant = TeachingAssistant.find_by(id: login.to_i)
            if assistant.nil?
                return nil
            end
            assistant if assistant.valid_password?(password)
